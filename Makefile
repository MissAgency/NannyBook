...
ifneq "$(SUPPORTS_MAKE_ARGS)" ""
  ...
  # Replace (escape) ":" by "\:" in COMMAND_ARGS to avoir error when execute console commande. ex "make console cache:clear"
  $(eval $(subst :,\:,$(COMMAND_ARGS)):;@:)
endif
...
composer:
	...
	@docker run -ti --rm=true \
		...
		numerique1/composer-hhvm bash -ci '\
			$(CREATE_USER_COMMAND) \
			$(ADD_SSH_ACCESS_COMMAND) \
...
bower:
	@docker run -ti --rm=true \
		...
		marmelab/bower bash -ci '\
			...
			$(ADD_SSH_ACCESS_COMMAND) \
			$(AUTHORIZE_HOME_DIR_COMMAND) \
			$(EXECUTE_AS) git config --global url."https://".insteadOf git:// && \
